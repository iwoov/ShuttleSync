...
### 场景1：抢热门时段
```
用户希望预约周五晚上19:00的场地，但该时段经常被秒抢。
用户可以：
1. 创建捡漏任务，设置扫描间隔为5分钟
2. 指定venue_site_id和reservation_time为"19:00"
3. 不指定site_name（接受任何场地）
4. 系统每5分钟检查一次，发现有人取消立即预约
```
...
用户只想预约"1号场"，不接受其他场地。
用户可以：
1. 创建捡漏任务，指定site_name为"1号场"
2. 设置较长的扫描间隔（如30分钟）节省资源
3. 系统只在"1号场"可用时才进行预约
```
...
用户可以：
1. 创建捡漏任务，不指定site_name和reservation_time
2. 系统会预约任何可用的场地和时间
3. 提高预约成功率
```
...
捡漏模式完全复用了现有的预约逻辑和API接口，只是在上层增加了定时扫描和自动触发的机制。核心优势是：
- ✅ 无需前端人工监控
- ✅ 提高预约成功率（两个账号）
- ✅ 灵活的过滤条件
- ✅ 完整的日志追踪
